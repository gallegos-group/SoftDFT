...
function pac_continuation_graft(input_file :: String)

    dataset = YAML.load_file(input_file)

    return pac_continuation_graft(dataset)
end

function pac_continuation_graft(dataset :: Dict{Any, Any})
    ...
    cont_var = [log(dataset["species"][1]["fixed"]["density"])]
    ...
    continuation_function_graft(problem_data, solution_variables, numerics, cont_var)
end

function continuation_function_graft(problem_data :: Tuple, solution_variables :: Tuple, numerics :: Dict, cont_var :: Vector{Float64})

    ni = map(count_total_entries, solution_variables)
    n = sum(ni) + 1 # for continuation variable

    num_keep = get(numerics, "num_keep", 5)
    ds_init = get(numerics, "ds_init", 0.5e-1)
    ds_min = get(numerics, "ds_min", 1e-3)
    ...
    update_cont_var_graft(dataset, cont_var)
    ...
    results = NamedTuple[]  # (step, graft_dens, brush_height, alpha)

    open("pac_output.txt", "w") do io
        out_root = "graft_sweep"
        root_abs = abspath(out_root)  # <-- absolute root
        mkpath(root_abs)
        original_pwd = pwd()

        while cont_var[1] < 1.0 #cont_var[1] > 1e-10 && cont_var[1] < 0.21
            ...
            graft_dens = exp(cont_var[1])
            graft_dir = joinpath(out_root, _format_dir(graft_dens; digits=7))
            mkpath(graft_dir)
            ...
            cd(graft_dir) do
                ...
                r = (step=step, graft_dens=graft_dens, alpha=alpha_val, brush_height=brush_height_val)
                push!(results, r)
                ...
                open("run_info.csv", mode) do io
                    if step == 3
                        println(io, "step,graft_dens,alpha,brush_height")
                    end
                    println(io, "$i,$graft_dens,$alpha_val,$brush_height_val")
                end
            ...
            if step % print_frequency == 0
                println("PAC Step $step | Continuation param = $(exp(cont_var[1]))")
            end
...
function objective_pac_graft(problem_data :: Tuple)
...
function update_cont_var_graft(dataset, cont_var:: Vector{Float64})
    dataset["species"][1]["fixed"]["density"] = exp(cont_var[1])
end
...
function _append_summary_line(root_abs::AbstractString, r)
    ...
    open(path, header_needed ? "w" : "a") do io
        if header_needed
            println(io, "step,graft_dens,brush_height,alpha")
        end
        @printf(io, "%d,%.12g,%.12g,%.12g\n", r.step, r.graft_dens, r.brush_height, r.alpha)
        flush(io)
    end
end

# Helper: consistent folder names like dist_1.60, dist_1.62, ...
_format_dir(d; digits=7) = "graft_" * @sprintf("%0.*f", digits, d)